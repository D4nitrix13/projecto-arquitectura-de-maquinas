...
function obtenerRutaBaseProyecto(): string
{
    $scriptName = str_replace("\\", "/", $_SERVER["SCRIPT_NAME"] ?? "");
    $posicionPages = strpos($scriptName, "/pages/");

    if ($posicionPages !== false) {
        return substr($scriptName, 0, $posicionPages);
    }

    $posicionTutoriales = strpos($scriptName, "/tutoriales/");

    if ($posicionTutoriales !== false) {
        return substr($scriptName, 0, $posicionTutoriales);
    }

    $posicionPracticas = strpos($scriptName, "/practicas/");

    if ($posicionPracticas !== false) {
        return substr($scriptName, 0, $posicionPracticas);
    }
...
function protegerAccesoUnidad(): void
{
    ...
    $redirigirA = $rutaBase . $unidades[$archivoActual]["redirigir_a"];
